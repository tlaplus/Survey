...
## Cleanup/Bin country names in column 10: US, USA, Usa, United States, ... (https://www.journaldev.com/43690/sub-and-gsub-function-r).
data[c(10)] <- lapply(data[c(10)], gsub, pattern = "^US|^United States|^usa|^United States of America|^USA ", replacement = "USA", ignore.case = TRUE)
data[c(10)] <- lapply(data[c(10)], gsub, pattern = "^USA ", replacement = "USA", ignore.case = TRUE)
data[c(10)] <- lapply(data[c(10)], gsub, pattern = "^Britain$|^United Kingdom$", replacement = "UK", ignore.case = TRUE)
data[c(10)] <- lapply(data[c(10)], gsub, pattern = "^india$", replacement = "India", ignore.case = TRUE)
data[c(10)] <- lapply(data[c(10)], gsub, pattern = "^germany|^Germany ", replacement = "Germany", ignore.case = TRUE)
data[c(10)] <- lapply(data[c(10)], gsub, pattern = "^Netherlands ", replacement = "Netherlands", ignore.case = TRUE)
data[c(10)] <- lapply(data[c(10)], gsub, pattern = "^Italy ", replacement = "Italy", ignore.case = TRUE)
data[c(10)] <- lapply(data[c(10)], gsub, pattern = "^switzerland$", replacement = "Switzerland", ignore.case = TRUE)
data[c(10)] <- lapply(data[c(10)], gsub, pattern = "^$", replacement = "Unknown", ignore.case = TRUE)

library(dplyr)
library(ggplot2)
ggplot(data %>% count(Country), aes(x = reorder(Country, -n), y = n, fill = Country)) + 
